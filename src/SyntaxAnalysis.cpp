...
void SyntaxAnalysis::fillMemoryVarList(Token& t1, Token& t2)
{
	...
	Variable* var = new Variable(memName, 0, Variable::MEM_VAR, stoi(t2.getValue()));
...
/*Returns position of instruction*/
...
void SyntaxAnalysis::fillSuccesors()
{
	...
	{
		//If instruction is branch it finds position of succesor that is supposed to branch to 
...
void SyntaxAnalysis::createMIPS()
{
	...
	MIPSfile << ".text\n";
		...
		for (Instructions::iterator it = instructions.begin(); it != instructions.end(); it++)
		{
			...
			{
			case I_ADD:
				MIPSfile << "\tadd\t\t $" << returnAssignedReg((*it)->m_dst.front()->GetName()) << ", $" << returnAssignedReg((*it)->m_src.front()->GetName()) << ", $" << returnAssignedReg((*it)->m_dst.back()->GetName()) <<endl;
				break;
			case I_SUB:
				MIPSfile << "\tsub\t $" << returnAssignedReg((*it)->m_dst.front()->GetName()) << ", $" << returnAssignedReg((*it)->m_src.front()->GetName()) << ", $" << returnAssignedReg((*it)->m_dst.back()->GetName()) << endl;
				break;
			case I_ADDI:
				...
				break;
			case I_LW:
				MIPSfile << "\tlw\t\t $" << returnAssignedReg((*it)->m_dst.front()->GetName()) << ", " << (*it)->m_src.front()->getValue() << "($"  << returnAssignedReg((*it)->m_src.back()->GetName()) << ")" << endl;
				break;
			case I_LA:
			...
			case I_BLTZ:
				...
				break;
			case I_OR:
				MIPSfile << "\tor\t\t $" << returnAssignedReg((*it)->m_dst.front()->GetName()) << ", $" << returnAssignedReg((*it)->m_src.front()->GetName()) << ", $" << returnAssignedReg((*it)->m_dst.back()->GetName()) << endl;
				break;
			case I_NOR:
				MIPSfile << "\tnor\t\t $" << returnAssignedReg((*it)->m_dst.front()->GetName()) << ", $" << returnAssignedReg((*it)->m_src.front()->GetName()) << ", $" << returnAssignedReg((*it)->m_dst.back()->GetName()) << endl;
				break;
			case I_BEQZ:
				MIPSfile << "\tbltz\t $" << returnAssignedReg((*it)->m_src.front()->GetName()) << ", " << (*it)->m_dst.front()->GetName() << endl;
				break;
